...
func (req SelectRequest[R]) Filter(f filter.Filter) SelectRequest[R] {
	req.query.Filter = filter.And(req.query.Filter, f)
	return req
}

// FilterField returns a new request with the specified field filter added to
// existing filters with logical AND.
func (req SelectRequest[R]) FilterField(path string, cmp filter.Comparison) SelectRequest[R] {
	req.query.Filter = filter.And(req.query.Filter, filter.Field(path, cmp))
	return req
}
...
func (req SelectRequest[R]) Include(relationships ...string) SelectRequest[R] {
	a := make([]string, len(req.includes)+len(relationships))
