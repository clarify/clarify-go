...
// Evaluate describe the instruction for performing an evaluation request as
// well as annotations for passing on to actions.
type Evaluation struct {
	// Annotations allows specifying event meta-data that's passed on to
	// actions.
...
func (e EvaluateActions) Do(ctx context.Context, cfg *Config) error {
	...
	result := EvaluateResult{
		Annotations: maps.Clone(e.Evaluation.Annotations),
		Data:        selection.Data,
	}
	logger.LogAttrs(
...
// action is responsible for checking cfg.DryRun() to determine if it should
// actually perform the action, or just log it to the console. It is also
// responsible for logging errors. The return value must be true if the next
// action in the chain should run and false otherwise.
...
// RoutineAction returns a new action function that runs the passed-in routine.
// If the routine succeed, the action returns true. If it fails, the error is
// logged and the action returns false.
func RoutineAction(r Routine) ActionFunc {
